...
class DGCPF_Ajax
{
    ...
    public function filter_products_handler()
    {
        ...
        if ( ! empty( $acf_meta_query_repeater ) && function_exists( 'get_field_object' ) ) {
            foreach ( $acf_meta_query_repeater as $meta_item ) {
                if ( ! empty( $meta_item['acf_meta_key'] ) && isset( $meta_item['acf_meta_value'] ) ) {
        ...
        if (!empty($selected_acf_fields)) {
            ...
            foreach ($selected_acf_fields as $field_key => $field_value) {
                if ($field_value !== '' && $field_value !== null && !empty($field_value)) {
                    ...
                    if ($field_object) {
                        $compare = ($field_object['type'] === 'text') ? 'LIKE' : '=';
                        $type = ($field_object['type'] === 'number') ? 'NUMERIC' : 'CHAR';
                        
                        if (is_array($field_value)) {
                            $checkbox_relation = ($filter_logic === 'OR') ? 'OR' : 'AND';
                            $checkbox_group = ['relation' => $checkbox_relation];
                            foreach($field_value as $value_item) {
                                $checkbox_group[] = [ 'key' => $field_key, 'value' => '"' . $value_item . '"', 'compare' => 'LIKE' ];
                            }
                            $meta_queries_from_filters[] = $checkbox_group;
                        } else {
        ...
        $available_filter_options = $this->get_available_filter_options_for_query($args, $filters_data, $selected_terms_by_taxonomy, $selected_acf_fields);
    ...
    private function get_available_filter_options_for_query($current_query_args, $filters_config, $selected_terms, $selected_acf) {
        $available_options = [];
        unset($current_query_args['paged']);
        unset($current_query_args['posts_per_page']);
        $current_query_args['fields'] = 'ids';
        $current_query_args['no_found_rows'] = true;
        ...
        foreach ($filters_config as $filter_config) {
            ...
            if ('taxonomy' === $filter_type) {
                ...
                if (empty($taxonomy_name) || !taxonomy_exists($taxonomy_name)) continue;
                ...
                foreach ($all_terms as $term) {
                    $count_args = $current_query_args;
                    
                    // Temporarily remove the current filter from the main query to count its own options
                    if (isset($count_args['tax_query'])) {
                        $count_args['tax_query'] = array_filter($count_args['tax_query'], function($tax_query_item) use ($taxonomy_name) {
                            return is_array($tax_query_item) && (!isset($tax_query_item['taxonomy']) || $tax_query_item['taxonomy'] !== $taxonomy_name);
                        });
                    }

                    $count_args['tax_query'][] = ['taxonomy' => $taxonomy_name, 'field' => 'slug', 'terms' => $term->slug, 'operator' => 'IN'];
                    
                    $count_query = new WP_Query($count_args);
                    $available_options[$taxonomy_name][$term->slug] = ['name' => $term->name, 'count' => $count_query->post_count];
                }
                ...
                if (in_array($field_object['type'], ['select', 'radio', 'checkbox', 'true_false'])) {
                    ...
                    foreach ($choices as $value => $label) {
                        ...
                        // Temporarily remove the current ACF filter from the main query
                        if (isset($count_args['meta_query'])) {
                            $count_args['meta_query'] = array_filter($count_args['meta_query'], function($meta_query_item) use ($acf_field_key) {
                                return is_array($meta_query_item) && (!isset($meta_query_item['key']) || $meta_query_item['key'] !== $acf_field_key);
                            });
                        }
                        
                        $count_args['meta_query'][] = ['key' => $acf_field_key, 'value' => $value, 'compare' => '='];
                        
                        $count_query = new WP_Query($count_args);
                        $available_options[$acf_field_key]['values'][$value] = ['name' => $label, 'count' => $count_query->post_count];
                    }
                }
            }
        }
